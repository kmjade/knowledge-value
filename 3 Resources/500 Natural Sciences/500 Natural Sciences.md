---
aliases:
  - 自然科学
  - Natural Sciences
  - DDC 500
created: 2026-05-28
updated: 2026-05-28
type: directory-index
topic: natural-sciences
tags:
  - resources
  - natural-sciences
  - ddc-500
---

# NATURAL SCIENCES / 自然科学

> [!info] DDC Class 5 · Natural Sciences & Mathematics
> 研究自然世界的结构、规律与现象的学科大门。从数学的抽象逻辑到生物的演化之树。
> 本目录是 DDC Class 5 的根入口，当前以数学为核心，其余子学科待建设。

---

## 📂 目录结构

```
500 Natural Sciences/                       ← 你在这里
│
├── 500 Natural Sciences.md                 ← 本文件：DDC 5 入口索引
│
├── 510-Mathematics/          🟢 已建设      ← 数学知识库 (DDC 510)
│   ├── README.md                            17 文件：基础数学·高等数学·统计·黎曼猜想
│   ├── 基础数学/
│   ├── 高等数学/
│   ├── 统计学与概率/
│   ├── 应用数学/
│   └── 学习资源/
│
├── 520-Astronomy/            🟡 待建设      ← 天文学 (DDC 520)
├── 530-Physics/              🟡 待建设      ← 物理学 (DDC 530)
├── 540-Chemistry/            🟡 待建设      ← 化学 (DDC 540)
├── 550-Earth-Sciences/       🟡 待建设      ← 地球科学 (DDC 550)
├── 560-Paleontology/         🟡 待建设      ← 古生物学 (DDC 560)
├── 570-Life-Sciences/        🟡 待建设      ← 生命科学 (DDC 570)
├── 580-Plants/               🟡 待建设      ← 植物学 (DDC 580)
└── 590-Animals/              🟡 待建设      ← 动物学 (DDC 590)
```

---

## 🗺️ DDC Class 5 全景

| DDC     | 领域                                        | 本库状态 |
| ------- | ------------------------------------------- | :------: |
| **500** | Natural sciences 总论                       | 🟢 本页 |
| **510** | Mathematics 数学                            | 🟢 [[510-Mathematics/README\|→ 进入]] (17 文件) |
| **520** | Astronomy 天文学                            | 🟢 [[520-Astronomy/README\|→ 11文件]] |
| **530** | Physics 物理学                              | 🟢 [[530-Physics/README\|→ 11文件]] |
| **540** | Chemistry 化学                              | 🟢 [[540-Chemistry/README\|→ 11文件]] |
| **550** | Earth sciences & geology 地球科学           | 🟢 [[550-Earth-Sciences/README\|→ 11文件]] |
| **560** | Fossils & prehistoric life 古生物学         | 🟢 [[560-Paleontology/README\|→ 11文件]] |
| **570** | Life sciences / Biology 生命科学            | 🟢 [[570-Life-Sciences/README\|→ 11文件]] |
| **580** | Plants (Botany) 植物学                      | 🟢 [[580-Plants/README\|→ 11文件]] |
| **590** | Animals (Zoology) 动物学                    | 🟢 [[590-Animals/README\|→ 11文件]] |

---

## 🧭 按意图导航

| 我想... | 去这里 |
|---------|--------|
| 🔢 **系统学习数学** | [[510-Mathematics/README\|510 Mathematics]] → 从基础到高等 |
| 🚀 **探索天文学** | [[520-Astronomy/README\|DDC 520 天文学]] (11文件) |
| ⚛️ **了解物理学** | [[530-Physics/README\|DDC 530 物理学]] (11文件) |
| 🧬 **学习生命科学** | [[570-Life-Sciences/README\|DDC 570 生命科学]] (11文件) |
| 🔬 **看应用如何落地** | [[3 Resources/06 Applied Sciences/06 Applied Sciences\|DDC 600 应用科学]] — 科学→技术→工程 |

---

## ① 510-Mathematics/ — 数学知识库 🟢

> 从基础代数、几何到高等微积分、线性代数，再到黎曼猜想专题。17 文件。

### 模块速查

| 模块 | 内容 | 文件 |
|------|------|:----:|
| [[510-Mathematics/基础数学/自然数与整数\|基础数学]] | 代数方程、函数基础、平面几何、数论 | 4 |
| [[510-Mathematics/高等数学/微积分基础\|高等数学]] | 微积分、线性代数 | 2 |
| [[510-Mathematics/统计学与概率/概率论基础\|统计学与概率]] | 概率论基础 | 1 |
| [[510-Mathematics/应用数学/机器学习中的数学\|应用数学]] | 机器学习中的数学 | 1 |
| [[510-Mathematics/黎曼猜想知识库\|黎曼猜想]] | ζ 函数、素数分布、专题 | 5 |
| [[510-Mathematics/学习资源/书籍推荐\|学习资源]] | 书籍、在线课程 | 2 |

> 完整入口：[[510-Mathematics/README]]

---

## ② 自然科学 → 应用科学

基础科学（DDC 500）回答"为什么"，应用科学（DDC 600）回答"怎么做"：

```
基础科学                应用科学                 工程实践
(Basic Science)    (Applied Sciences)      (Engineering)
     ↓                    ↓                      ↓
   数学              →   控制论/运筹学     →   自动驾驶
   物理学            →   材料科学          →   芯片制造
   化学              →   化学工程          →   制药工艺
   生物学            →   生物技术          →   CRISPR 疗法
```

> 完整科学-技术-工程光谱参见 [[3 Resources/06 Applied Sciences/06 Applied Sciences|DDC 600 应用科学]]

---

## 🔗 跨库连接

### 父节点
- [[3 Resources/3 Resources.md|Resources 资源总览]]

### 关联知识库

| 知识库 | 关系 |
|--------|------|
| [[3 Resources/06 Applied Sciences/06 Applied Sciences\|DDC 600 应用科学]] | 下游：科学→技术→工程 |
| [[3 Resources/0 Department/Computer-Science/Computer-Science\|计算机科学]] | 数学是计算的理论基础 |
| [[3 Resources/000 Knowledge/000 Knowledge\|000 Knowledge (DDC 001)]] | 知识组织系统 |
| [[100 Philosophy & Psychology\|DDC 100 哲学]] | 科学哲学、认识论 |

---

## ✅ 维护清单

### 日常
- [ ] 新数学资料归档到 `510-Mathematics/99-资源收集/`
- [ ] 新自然科学资料按 DDC 子分类归档

### 按需
- [x] 启动 Physics (DDC 530) [[530-Physics/README\|→ 11文件]] 子库建设
- [x] 启动 Life Sciences (DDC 570) [[570-Life-Sciences/README\|→ 11文件]] 子库建设
- [ ] 从 `0 Department/` 整合数学/物理/化学相关内容

---

## 📅 更新日志

| 日期 | 更新内容 |
|------|----------|
| 2026-05-28 | 创建 DDC 5 入口 |
| 2026-05-28 | **迁移**：`02-Learning/数学知识库/` → `510-Mathematics/` (17 文件) |

---

*分类: 3 Resources/500 Natural Sciences · DDC: 500 Natural Sciences & Mathematics*
