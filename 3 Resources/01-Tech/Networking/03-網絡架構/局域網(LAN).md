---
title: 局域网(LAN)
aliases:
  - Local Area Network
  - 以太网
  - LAN
para: resource
tags:
  - #para/resource/tech
  - #topic/networking
  - #protocol/lan
  - #type/concept
  - #zettel/type/permanent
created: 2026-01-21
---

> [!summary] 局域网概述
> LAN (Local Area Network) 是在有限地理范围内（如办公室、校园、建筑物）連接計算机的網路。

---

## 🏢 LAN基礎

### 定义

局域网是在较小的地理范围内（通常是几米到几公里）連接計算机、列印机、伺服器等设备的網路。

### 特點

| 特性 | 說明 |
|------|------|
| 地理范围 | 小（房间、楼宇、校园） |
| 傳輸速率 | 高（10Mbps - 100Gbps） |
| 误码率 | 低 |
| 介质 | 双绞线、光纤、無线 |
| 拓扑结构 | 星型（最常见） |

### 應用程式場景

- 家庭網路
- 办公室網路
- 校园網路
- 工厂網路
- 數據中心内部網路

---

## 🔌 以太网標準

### 以太网速度

| 標準 | 速度 | 介质 | 最大距离 |
|------|------|------|----------|
| 10Base5 | 10 Mbps | 粗同轴电缆 | 500m |
| 10Base2 | 10 Mbps | 细同轴电缆 | 185m |
| 10BaseT | 10 Mbps | 双绞线（Cat3） | 100m |
| 100BaseT | 100 Mbps | 双绞线（Cat5） | 100m |
| 1000BaseT | 1 Gbps | 双绞线（Cat5e） | 100m |
| 10GBaseT | 10 Gbps | 双绞线（Cat6a） | 100m |
| 1000BaseSX | 1 Gbps | 光纤（多模） | 550m |
| 1000BaseLX | 1 Gbps | 光纤（单模） | 5km |
| 10GBaseSR | 10 Gbps | 光纤（多模） | 300m |
| 10GBaseLR | 10 Gbps | 光纤（单模） | 10km |

### 双绞线分類

| 类别 | 频率 | 應用程式 |
|------|------|------|
| Cat3 | 16 MHz | 10BaseT（已淘汰） |
| Cat5 | 100 MHz | 100BaseT |
| Cat5e | 100 MHz | 1000BaseT |
| Cat6 | 250 MHz | 10GBaseT（短距离） |
| Cat6a | 500 MHz | 10GBaseT（100m） |
| Cat7 | 600 MHz | 10GBaseT+ |
| Cat8 | 2000 MHz | 25G/40GBaseT |

---

## 🏗️ LAN拓扑設計

### 小型办公室網路

```
                     [路由器]
                        │
              ┌─────────┴─────────┐
              │    [核心交换机]   │
              └─────────┬─────────┘
                        │
        ┌───┬───┬───┬───┴───┬───┬───┐
        │   │   │   │       │   │   │
       PC  PC  PC  PC     列印机  AP  伺服器
```

### 中型企业網路

```
                   [出口路由器]
                        │
                 ┌──────┴──────┐
                 │ [防火墙]    │
                 └──────┬──────┘
                        │
                ┌───────┴───────┐
        ┌───────┤ 核心交换机   ├───────┐
        │       └───────┬───────┘       │
   ┌────┴────┐          │          ┌────┴────┐
   │ 汇聚层  │          │          │ 汇聚层  │
   ├─┬─┬─┬─┤          │          ├─┬─┬─┬─┤
   │ │ │ │ │      ┌───┴───┐      │ │ │ │ │
  PC PC PC PC PC   伺服器群  PC PC PC PC PC
```

---

## 🔌 交换机

### 工作原理

交换机工作在數據鏈路层，通過MAC地址表轉發數據帧。

### MAC地址表

```
端口        MAC地址            老化時間
Port 1      AA:BB:CC:DD:EE:01  300s
Port 2      AA:BB:CC:DD:EE:02  300s
Port 3      AA:BB:CC:DD:EE:03  300s
```

### 交换机轉發

| 情况 | 處理方式 |
|------|----------|
| 目標MAC已知 | 单播到对应端口 |
| 目標MAC未知 | 广播到所有端口（除源端口） |
| 目標MAC为广播/组播 | 广播/组播到所有端口 |
| 目標MAC为交换机本身 | 接收 |

---

## 🏷️ VLAN

### 定义

VLAN (Virtual Local Area Network) 将一个物理LAN划分成多個逻辑LAN。

### 優勢

| 優勢 | 說明 |
|------|------|
| 安全性 | 隔離廣播域 |
| 效能 | 減少廣播流量 |
| 靈活性 | 不受物理位置限制 |
| 管理簡化 | 減少網絡擁塞和管理複雜度 |

### VLAN类型

| 类型 | 說明 |
|------|------|
| 基于端口 | 将端口分配到VLAN |
| 基于MAC | 根据MAC地址分配 |
| 基于協議 | 根據協議類型分配 |
| 基於子網 | 根據IP子網分配 |

### VLAN配置示例

```bash
# 創建VLAN
vlan 10
 name Sales
vlan 20
 name Engineering

# 分配端口到VLAN
interface fastethernet 0/1
 switchport mode access
 switchport access vlan 10

# Trunk端口（承载多個VLAN）
interface fastethernet 0/24
 switchport mode trunk
 switchport trunk allowed vlan 10,20
```

---

## 🔁 冗余設計

### STP (生成树协议)

STP防止二层环路，避免广播风暴。

**STP端口狀態**：
1. **Blocking** - 阻塞狀態，不轉發數據
2. **Listening** - 监听狀態，选举根桥
3. **Learning** - 學習狀態，學習MAC地址
4. **Forwarding** - 轉發狀態，正常轉發
5. **Disabled** - 禁用狀態

**STP变体**：
- **STP** (802.1D) - 传统STP
- **RSTP** (802.1w) - 快速STP
- **MSTP** (802.1s) - 多生成树协议

### VRRP (虚拟路由器冗余协议)

VRRP實現网关冗余，提高網路可靠性。

```
    [VRRP网关：192.168.1.254]
          │
   ┌──────┴──────┐
   │             │
[路由器A]      [路由器B]
(Master)       (Backup)
```

---

## 🎯 LAN設計最佳實踐

### 1. 分层設計

```
核心层
  ├─ 高速交换（10Gbps+）
  ├─ 冗余設計
  └─ 最少策略

汇聚层
  ├─ 路由功能
  ├─ 策略实施
  └─ 流量聚合

接入层
  ├─ 使用者接入
  ├─ 端口安全
  └─ VLAN划分
```

### 2. 冗余設計

- 双核心交换机
- 双上行鏈路
- 冗余网关（VRRP/HSRP）
- 冗余电源

### 3. 安全措施

- VLAN隔离
- 端口安全（限制MAC数量）
- DHCP Snooping
- 動態ARP检测
- ACL（访问控制列表）

---

## 📝 學習檢查清單

- [ ] 理解LAN的定义和特點
- [ ] 知道常见的以太网標準
- [ ] 了解交換機的工作原理
- [ ] 掌握VLAN配置
- [ ] 了解STP防止環路
- [ ] 知道LAN設計的最佳實踐

---

## 🔗 相關連結

- 🔙 [[03-網絡架構|返回網絡架構]]
- 🏷️ [[VLAN虛擬局域網]]
- 🔌 [[交換機]]
