...
| 2️⃣ 確認網路介面已連線      | - 查看 Wi‑Fi 或有線網路圖示是否顯示已連線 <br> - 若使用 VPN，暫時關閉                                                          | 防止路由器流量被其他介面或 VPN 攔截                          |
...
| 6️⃣ 檢查路由器的 ICMP 設定 | 部分路由器會在管理介面關閉 ICMP 回應 <br> - 登入路由器管理頁面（通常是 http://192.168.1.1） <br> - 確認「允許 Ping」或「ICMP Echo Reply」已啟用 | 若路由器本身不回應，ping 會一直超時                          |
| 7️⃣ 嘗試其他網路工具       | - `tracert 192.168.1.1`（Windows）或 `traceroute`（macOS/Linux）<br> - `arp -a` 查看是否能看到路由器的 MAC 位址          | 進一步定位是「無法到達」還是「被拒絕」                           |
...
> **提示**：使用 QuickAdd 宏 **🤖 Claude Code → 📋 请求** 可以把這個筆記直接送給 Claude 進一步分析或產生自動化腳本（例如 `networkHelper.js`）。
